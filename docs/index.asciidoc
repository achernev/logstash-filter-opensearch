...
Authorization to a secure OpenSearch cluster requires `read` permission at index level and `monitoring` permissions at cluster level.
...
Authenticate using OpenSearch API key. Note that this option also requires
enabling the <<plugins-{type}s-{plugin}-ssl_enabled>> option.

Format is `id:api_key` where `id` and `api_key` are as returned by the
OpenSearch {ref}/security-api-create-api-key.html[Create API key API].
...
The SHA-256 fingerprint of an SSL Certificate Authority to trust, such as the autogenerated self-signed CA for an OpenSearch cluster.
...
OpenSearch query string. More information is available in the
{ref}/query-dsl-query-string-query.html#query-string-syntax[OpenSearch query
...
File path to opensearch query in DSL format. More information is available in
the {ref}/query-dsl.html[OpenSearch query documentation].
Use either `query` or `query_template`.
...
Enable SSL/TLS secured communication to OpenSearch cluster.
...
List of allowed SSL/TLS versions to use when establishing a connection to the OpenSearch cluster.
...
==== OpenSearch Filter Deprecated Configuration Options
